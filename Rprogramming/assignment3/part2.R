...
texas <- outcome[outcome$State == "MD" & !is.na(outcome[,11]),]
orderHA <- order(texas[,11], texas[, 2])
texasOrdered <- texas[orderHA,]
r <- rank(texasOrdered[,11])
seq(1:nrow(texasOrdered))
texasOrdered$RankHeartFailure <- seq(1:nrow(texasOrdered))
texasOrdered$Hospital.Name[texasOrdered$RankHeartFailure == 1]
texasOrdered$Hospital.Name[texasOrdered$RankHeartFailure == 5000]
